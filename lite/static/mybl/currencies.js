// Исправление фона таблиц
/*document.addEventListener('DOMContentLoaded', function() {
    // Ждём загрузки DOM
    setTimeout(function() {
        // Принудительно устанавливаем тёмный фон всем таблицам
        document.querySelectorAll('table').forEach(table => {
            table.style.backgroundColor = '#343a40';
            //table.style.color = 'white';
            
            // Все ячейки тоже тёмные
            table.querySelectorAll('td, th').forEach(cell => {
                cell.style.backgroundColor = '#343a40';
                cell.style.borderColor = '#454d55';
            });
        });
        // Теперь применяем раскраску
        setTimeout(colorCurrencyTables, 100);
    }, 500);
});*/
...
// Раскраска таблицы валют
function colorCurrencyTables() {
    ...
    [valuePlus, valuePlus1, valuePlus2, valuePlus3].forEach(values => {
        if (values.length > 0) {
            let firstVal = parseFloat(values[0].textContent) || 1;
            let lastVal = parseFloat(values[values.length - 1].textContent) || -1;
            
            values.forEach((cell, i) => {
                let val = parseFloat(cell.textContent) || 0;
                if (val >= 0) {
                    cell.style.backgroundColor = 'rgba(40, 167, 69,' + (val / (firstVal * 2)) + ')';
                } else {
                    cell.style.backgroundColor = 'rgba(220, 53, 69,' + (val / (lastVal * 1.5)) + ')';
                }
            });
        }
    });
    ...
    // Классификация валют
    if (currencyPlus.length > 0) {
        currencyPlus.forEach(cell => {
            let match = cell.textContent.match(/[A-Z]\w+/);
            if (match) {
                let currency = match[0];
                
                if (['USD', 'CNY', 'EUR'].includes(currency)) {
                    cell.classList.add('bg-info', 'text-dark', 'font-weight-bold');
                }
                if (['TRY', 'ZAR', 'UAH', 'BRL', 'KZT', 'INR'].includes(currency)) {
                    cell.classList.add('bg-warning', 'font-weight-bold');
                }
                if (['NOK', 'ZAR', 'AUD', 'CAD', 'KZT', 'BRL'].includes(currency)) {
                    cell.classList.add('text-light', 'font-weight-bold', 'font-italic', 'text-lowercase');
                }
                if (['KRW', 'PLN'].includes(currency)) {
                    cell.classList.add('bg-secondary', 'text-warning');
                }
                if (currency === 'XDR') {
                    cell.classList.add('bg-primary', 'font-weight-lighter');
                }
                if (currency === 'CNY') {
...
/*function fixMoexTableColors() {
    const table = document.getElementById('moex');
    if (!table) return;
    
    // Сбрасываем все inline-стили фона
    table.querySelectorAll('td').forEach(td => {
        td.style.backgroundColor = '';
    });
    
    // Устанавливаем только цвет текста
    table.querySelectorAll('.change-high, .change-low').forEach(cell => {
        const value = parseFloat(cell.textContent) || 0;
        
        if (value > 0) {
            cell.style.backgroundColor = `rgba(40, 167, 69, ${Math.min(value/10, 0.9)})`;
            cell.style.color = value > 5 ? 'white' : '#e8f5e9';
        } else if (value < 0) {
            cell.style.backgroundColor = `rgba(220, 53, 69, ${Math.min(Math.abs(value)/10, 0.9)})`;
            cell.style.color = value < -5 ? 'white' : '#fdecea';
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(fixMoexTableColors, 1000);
});*/
